...
<table class="table table-bordered datatable" id="table-1">
	...
	<tbody>
		<?php foreach ($statusSells as $key => $statusSell) : ?>
		<tr>
			<td><?php echo $statusSell['StatusSell']['id']?></td>
			<td><?php echo $statusSell['StatusSell']['name']?></td>
			<td>
				<?php if ($statusSell['StatusSell']['active']) :?>
					<?php 
						$class  = "success"; 
						$status = "ativo";
					?>	
			...
			<td>
				<?php echo $this->Html->link('<i class="entypo-pencil"></i> Editar', 
					array(
						'controller' => 'statusSells', 
						'action' => 'admin_edit', 
						$statusSell['StatusSell']['id']
					),
					array(
				...
				<?php echo $this->Form->postLink('<i class="entypo-cancel"></i> Deletar', 
					...
					"Deseja realmente deletar esse status?"
				);?>
			</td>
		</tr>
		<?php endforeach; ?>
