...
<table class="table table-bordered datatable" id="table-1">
	...
	<tbody>
		<?php foreach ($promotionalCodes as $key => $promotionalCode) :?>
		<tr>
			<td><?php echo $promotionalCode['PromotionalCode']['id']?></td>
			<td><?php echo $promotionalCode['PromotionalCode']['name']?></td>
			<td><?php echo $promotionalCode['PromotionalCode']['discount']?>%</td>
			<td><?php echo date('d/m/Y',strtotime($promotionalCode['PromotionalCode']['validate']));?></td>
			<td>
				<?php if ($promotionalCode['PromotionalCode']['active']) :?>
					<?php 
						$class  = "success"; 
						$status = "ativo";
					?>	
			...
			<td>
				<?php echo $this->Html->link('<i class="entypo-pencil"></i> Editar', 
					array(
						'controller' => 'promotionalCodes', 
						'action' => 'admin_edit', 
						$promotionalCode['PromotionalCode']['id']
					),
					array(
				...
				<?php echo $this->Form->postLink('<i class="entypo-cancel"></i> Deletar', 
					...
					"Deseja realmente deletar esse código promocional?"
				);?>
			</td>
		</tr>
		<?php endforeach; ?>
